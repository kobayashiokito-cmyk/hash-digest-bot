...
def collect_new_items() -> int:
    ...
    new_items = [item for item in fetched if item["id"] not in known_ids and item["id"] not in queued_ids]
    ...
    for selector in ['a[href$=".pdf"]', 'a[href*="pdf"]', 'button', 'a']:
        for i in range(min(page.locator(selector).count(), 20)):
            try:
                ...
                href = (locator.get_attribute("href") or "") if selector.startswith("a") else ""
                if "pdf" in label.lower() or "ダウンロード" in label or href.lower().endswith(".pdf") or "pdf" in href.lower():
                    path = try_download_by_locator(page, locator, service_id)
                    if path:
                        return path
            except Exception:
...
def push_line_message(text: str) -> None:
    ...
    payload = {"to": LINE_USER_ID, "messages": [{"type": "text", "text": text[:5000]}]}
